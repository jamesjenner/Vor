...
function processPanelMessages(server, id, content) {
  ...
  switch (id) {
    ...
    case Panel.MESSAGE_MOVE_PANEL:
      receivedMovePanel(server, content);
      processed = true;
      break;
...
function receivedMovePanel(server, content) {
  movePanel(content);
}
...
function sendMovePanel(panelId, sourceColumn, targetColumn, row) {
  server.sendMessage(Panel.MESSAGE_MOVE_PANEL, {
    id: panelId, 
...
//  content.targetColumn
  ...
  if(row > 0) {
    ...
    $(this.dragPanel).insertAfter("#" + rows[row].id);
  } else {
    $(this.dragPanel).prependTo("#" + "column" + content.targetColumn);
  }
}
