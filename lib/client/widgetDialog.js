...
function viewWidgetWizard(mode, widget, panelId) {
  ...
  var addWidgetWizardDialog = new BootBoxWiz({
    ...
    },                  
    {
      type: BootBoxWiz.TYPE_FIXED,
      title: "Step 3",
      preProcessingLogic: widgetDialogStyleLogic,
      content:
      '<div class="form-group">' +
...
function widgetDialogDataSourceLogic(fields) {
  var i;
  var add = false;
  var dataSource = null;
...
function widgetDialogStyleLogic(fields) {
  var dataSource = null;
  ...
  if(dataSource !== null) {
    ...
    DataSource.DISPLAY_OPTIONS[dataSource.type][fields.widgetType];
    
    for(var type in DataSource.DISPLAY_OPTIONS[dataSource.type]) {
      if(type.display !== undefined && type.display !== null) {
        if(type.display instanceof Array) {
          add = false;
          for(i = 0; i < type.display.length; i++) {
            if(type.display[i] === dataSource.widgetType) {
              add = true;
              break;
            }
          }
