...
function viewWidgetWizard(mode, widget, panelId) {
  ...
  var addWidgetWizardDialog = new BootBoxWiz({
    ...
    stepContent: [ {
      ...
      '  <div class="col-md-4">' + 
      '    <select id="widgetDataSourcePicker" name="widgetDataSource" class="selectpicker">' +     
      dataSourcesHTML + 
      '    </select>' + 
      '  </div>' +
      '</div>',
    },
    {
      ...
      '  <div class="col-md-4">' + 
      '    <select id="widgetTypePicker" name="widgetType" class="selectpicker">' +     
      '    </select>' + 
      '  </div>' +
      '</div>' +
      '<div class="form-group">' +
      ...
      '  <div class="col-md-4">' + 
      '    <select id="widgetStylePicker" name="widgetStyle" class="selectpicker">' +     
      '    </select>' + 
      '  </div>' +
      '</div>' 
    },                  
  ...
  $("#widgetDataSourcePicker").on('change', function() {
    setWidgetDialogWidgetTypeValues($('.stepwizard .form-horizontal').serializeObject());
  });
  ...
  $("#widgetTypePicker").on('change', function() {
    ...
    setWidgetDialogWidgetStyleValues($('.stepwizard .form-horizontal').serializeObject());
  });
}
...
function setWidgetDialogWidgetTypeValues(fields) {
  var i;
  var add = false;
  var dataSource = null;
  ...
  for(i = 0; i < dataSources.length; i++) {
    if(dataSources[i].name === fields.widgetDataSource) {
  ...
  fields.widgetType = $('#widgetTypePicker').val();
  setWidgetDialogWidgetStyleValues(fields);
}

function setWidgetDialogWidgetStyleValues(fields) {
  var dataSource = null;
  ...
  if(dataSource !== null) {
    for(var type in DataSource.DISPLAY_OPTIONS[dataSource.type]) {
      if(DataSource.DISPLAY_OPTIONS[dataSource.type][type].display !== undefined && DataSource.DISPLAY_OPTIONS[dataSource.type][type].display !== null) {
        if(DataSource.DISPLAY_OPTIONS[dataSource.type][type].name === fields.widgetType) {
