...
//
 ...
 ["actual",98,172,188,176,170,160,146,146,146,146]]      
      ...
      // calculate goal, starting point is maximum estimate across all days
      ...
      massagedData[0][1] = maxEstimate;
      decrementFactor = maxEstimate / data.data.length;
      
      for(var i = 2, j = 1; i < data.data.length; i++, j++) {
        massagedData[0][i] = maxEstimate - (decrementFactor * j);
      }
      
      // TODO: see if way to use graph by plotting start and end point, not every single column
      // avoid rounding issues
      massagedData[0][data.data.length - 1] = 0;
      
      // populate actual, presume zeroed out for days after today
      for(var i = 1; i < data.data.length; i++) {
        massagedData[2][i] = parseInt(data.data[i].outstandingWork);
      }
      break;
  ...
  if(widget.style === Widget.DISPLAY_TYPE_GRAPH_CHART) {
      ...
      this.__updateWidgetGraphChartToDom(widget, massagedData, maxEstimate);
  }
};
