...
//
  ...
  switch(widget.type) {
    case DataSource.DISPLAY_OPTIONS[DataSource.TYPE_VERSION_ONE].BURN_DOWN.name:
      ...
      /*
       * TODO: fix goal, should only be last point and first point
       * can be resolved via multiple xy line chart, refer http://c3js.org/samples/simple_xy_multiple.html 
       * 
 ["goal",407,370,333,296,259,222,185,148,111,0],
 ["trend"],
 ["actual",98,172,188,176,170,160,146,146,146,146]]      
      ...
      massagedData[0][0] = 'goal';
      massagedData[1][0] = 'trend';
      massagedData[4]    = ['trendx', 1, 10];
      massagedData[5]    = ['goalx', 1, 10];
      massagedData[2][0] = 'actual';
      massagedData[3][0] = 'actualx';
      var actualData = [];
      var trendPoints;
      ...
      // setup goal
      massagedData[0][1] = maxOutstanding;
      massagedData[0][2] = 0;

      // x pos 0 is start of day 1, y is the estimate
      actualData[0] = parseInt(data.data[0].estimatedWork);
      // include 0 hour on sprint
//      massagedData[2][1] = parseInt(data.data[0].estimatedWork);
//      massagedData[3][1] = 0;
      
      // populate from received data
      for(var i = 0; i < data.data.length; i++) {
      // include 0 hour on sprint
//        massagedData[2][i + 2] = parseInt(data.data[i].outstandingWork);
//        massagedData[3][i + 2] = i + 1;

        // start from end of day one on sprint
        massagedData[2][i + 1] = parseInt(data.data[i].outstandingWork);
        massagedData[3][i + 1] = i + 1;
        
        actualData[i + 1] =  parseInt(data.data[i].outstandingWork);
      }
      
      // setup trend
      trendPoints = calcTrendLine(xAxisPoint, actualData);
      console.log("trendPoints: " + trendPoints);
      massagedData[1][1] = trendPoints[0][1];
      massagedData[1][2] = trendPoints[1][1];
      break;
  ...
  if(widget.style === Widget.DISPLAY_TYPE_GRAPH_CHART) {
      ...
      this.__updateWidgetGraphChartToDom(widget, massagedData, maxOutstanding);
  }
};
