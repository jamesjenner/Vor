...
WidgetManager.prototype.receivedData = function(data) {
  ...
  for(var i = 0; i < this.widgets.length; i++) {
    if(this.widgets[i].keyDataElement === data.keyDataElement) {
      ...
      if(dataSource !== null) {
        this.__updateWidgetData(this.widgets[i], dataSource.type, data);
      }
/* 
 * widget example
 *      
...
//  console.log(JSON.stringify(data, null, ' '));
  ...
  switch(dataSourceType) {
    case DataSource.TYPE_VERSION_ONE:
      console.log("updating widget '" + widget.title + "' from version one data");
      // TODO: widget elements need to be saved, so can be called to update values for the widget
      break;
