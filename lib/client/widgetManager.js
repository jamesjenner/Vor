...
WidgetManager.prototype.__updateVersionOneWidget = function(widget, data) {
  ...
  switch(widget.type) {
    case DataSource.DISPLAY_OPTIONS[DataSource.TYPE_VERSION_ONE].TOTAL_PROGRESS.name:
      console.log("setting value1: " + data.completedStoryPoints);
      console.log("setting value2: " + (data.completedStoryPoints + data.wipStoryPoints + data.backlogStoryPoints));

      var total = data.completedStoryPoints + data.wipStoryPoints + data.backlogStoryPoints;
      
      value1 = data.completedStoryPoints / total * 100;
      // TODO: use moment to determine current time against sprint duration vs end date to calculate target %
      // TODO: should the target be updated independent of the progress update?
      // value2 = data.completedStoryPoints + data.wipStoryPoints + data.backlogStoryPoints;
      break;
  
    case DataSource.DISPLAY_OPTIONS[DataSource.TYPE_VERSION_ONE].PROGRESS_BY_STATE.name:
      value1 = data.backlockStoryPoints;
  ...
  switch(widget.style) {
    ...
    case Widget.DISPLAY_TYPE_PROGRESS:
      console.log("calling __updateWidgetProgress(" + widget + ", " + value1 + ", " + value2 + ");");
...
WidgetManager.prototype.__updateWidgetProgress = function(widget, value, target) {
  console.log("setting progress gauge to target: " + target + ", value: " + value);
