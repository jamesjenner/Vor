 ...
 * ideal line (optional, non standard) - projected completed work required to complete todo for the day
      ...
      for(var i = 0; i < data.data.length; i++) {
        // start from end of day one on sprint
        massagedData[2][i + 1] = parseInt(data.data[i].workPerformed);
        massagedData[3][i + 1] = i + 1;
        
        massagedData[1][i + 1] = parseInt(data.data[i].estimatedWork);
        massagedData[4][i + 1] = i + 1;
        
        if(maxYValue < massagedData[1][i + 1]) {
          maxYValue = massagedData[1][i + 1];
        }
        
        // final goal is the last estimated work
        massagedData[0][2] = massagedData[1][i + 1];
      }
