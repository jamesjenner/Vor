...
//
  ...
  switch(widget.type) {
    case DataSource.DISPLAY_OPTIONS[DataSource.TYPE_VERSION_ONE].BURN_DOWN.name:
      ...
      massagedData[0][1] = 'trend';
      massagedData[0][2] = 'actual';
      
      // calculate goal, starting point is maximum estimate across all days
      for(var i = 0; i < data.data; i++) {
        if(data.data[i].estimate > maxEstimate) {
          maxEstimate = data.data[i].estimate;
        }
      }

      massagedData[0][1] = maxEstimate;
      decrementFactor = maxEstimate / data.sprintDuration;
      
      for(var i = 2, j = 1; i < data.sprintDuration; i++, j++) {
      ...
      massagedData[0][data.sprintDuration - 1] = 0;
      
      // populate actual, presume zeroed out for days after today
      for(var i = 1; i < data.sprintDuration; i++) {
        massagedData[2][i] = data.data[i].outstandingWork;
      }
      break;
  ...
  if(widget.style === Widget.DISPLAY_TYPE_GRAPH_CHART) {
      // this.__updateWidgetGraphDonutToDom(widget, [
      // TODO: parse the correct values
      this.__updateWidgetGraphChartToDom(widget, massagedData);
  }
};
...
WidgetManager.prototype.__updateWidgetGraphChartToDom = function(widget, data) {
  this.widgetControls[widget.id].load({
    columns: data
  });
};
